...
public class PythonXUnitSensor extends PythonReportSensor {
  ...
  private static void simpleMode(final SensorContext context, List<File> reports) throws XMLStreamException {
    ...
    if (testsCount > 0) {
      saveMeasure(context, CoreMetrics.TESTS, testsCount);
      saveMeasure(context, CoreMetrics.SKIPPED_TESTS, testsSkipped);
      saveMeasure(context, CoreMetrics.TEST_ERRORS, testsErrors);
      saveMeasure(context, CoreMetrics.TEST_FAILURES, testsFailures);
      saveMeasure(context, CoreMetrics.TEST_EXECUTION_TIME, testsTime);
    }
  }
  ...
  private static void saveMeasure(SensorContext context, Metric<Integer> metric, int value) {
  ...
  private static void saveMeasure(SensorContext context, Metric<Long> metric, long value) {
  ...
  private void saveMeasure(SensorContext context, InputFile inputFile, Metric<Integer> metric, int value) {
    context.<Integer>newMeasure()
      .on(inputFile)
      .forMetric(metric)
      .withValue(value)
  ...
  private void saveMeasure(SensorContext context, InputFile inputFile, Metric<Long> metric, long value) {
    context.<Long>newMeasure()
      .on(inputFile)
      .forMetric(metric)
      .withValue(value)
